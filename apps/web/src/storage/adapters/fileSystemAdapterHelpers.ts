...
const parseMarkdownMetaFromFile = async (
  ...
  try {
    ...
    const themeIdMatch = match[1].match(/^theme:\s*(.+)$/m);
    ...
    return {
      ...
      themeId: themeIdMatch ? parseQuotedValue(themeIdMatch[1]) : undefined,
      title: titleMatch ? parseQuotedValue(titleMatch[1]) : undefined,
      status: statusMatch ? parseQuotedValue(statusMatch[1]) : undefined,
      slug: slugMatch ? parseQuotedValue(slugMatch[1]) : undefined,
    };
  } catch {
    return {};
  }
...
export const scanDirectory = async (
  ...
  for await (const entry of dirHandle.values()) {
    ...
    if (entry.kind === "file" && entry.name.endsWith(".md")) {
      ...
      const { themeName, themeId, title, status, slug } =
        await parseMarkdownMetaFromFile(file);
      files.push({
        path: entryPath,
        name: entry.name,
        size: file.size,
