...
export class IndexedDBAdapter implements StorageAdapter {
  ...
  async listFiles(): Promise<FileItem[]> {
    ...
    const records = await new Promise<MetaRecord[]>((resolve, reject) => {
      const request = store.getAll();
      request.onerror = () => reject(request.error);
      request.onsuccess = () => resolve(request.result as MetaRecord[]);
    });
    const files = await Promise.all(
      records.map(async (item): Promise<FileItem> => {
        const file: FileItem = {
          path: item.path,
          name: item.path.split("/").pop() ?? item.path,
          updatedAt: item.updatedAt,
        };
        if (/(?:^|\/)article\.md$/i.test(item.path)) {
          try {
            const parsed = parseArticleDocument(await this.readFile(item.path));
            file.meta = {
              title:
                typeof parsed.frontmatter.title === "string"
                  ...
                  : undefined,
              themeName:
                typeof parsed.frontmatter.theme === "string"
                  ? parsed.frontmatter.theme
                  : undefined,
              themeId:
                typeof parsed.frontmatter.theme === "string"
                  ? parsed.frontmatter.theme
            ...
            // Keep malformed articles visible so the editor can repair them.
          }
        }
        return file;
      }),
  ...
  async writeFile(path: string, content: string): Promise<void> {
    if (/(?:^|\/)article\.md$/i.test(path) && (await this.exists(path))) {
      const current = await this.readFile(path);
      const directory = path.replace(/\\/g, "/").replace(/\/article\.md$/i, "");
      await this.writeFile(`${directory}/history/article.previous.md`, current);
    }
    if (!this.db) throw new Error("IndexedDB not initialized");
