...
export class FileSystemAdapter implements StorageAdapter {
  ...
  async writeFile(path: string, content: string): Promise<void> {
    if (/(?:^|\/)article\.md$/i.test(path) && (await this.exists(path))) {
      ...
      const { dir } = this.splitPath(this.normalizePath(path));
      const backupPath = dir
        ? `${dir}/history/article.previous.md`
        : "history/article.previous.md";
      const backupHandle = await this.resolveFileHandle(backupPath, true);
