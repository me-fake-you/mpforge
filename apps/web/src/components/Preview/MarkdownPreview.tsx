...
export function MarkdownPreview({
  ...
  const selectableThemes = useThemeStore(
    (state) => state.getAllThemes,
  )().filter(isThemeSelectable);
  const [previewScheme, setPreviewScheme] = useState<"light" | "dark">("light");
  const [previewWidth, setPreviewWidth] = useState<375 | 402>(402);
  ...
  useEffect(() => {
    ...
    const isDarkMode = previewScheme === "dark";
  ...
  return (
    <div
      ...
      style={
        { "--mpforge-preview-width": `${previewWidth}px` } as CSSProperties
      }
    >
      <div className="preview-header">
        <div className="preview-heading">
          <span className="preview-title">微信宽度实时预览</span>
          <span className="preview-subtitle">模拟预览</span>
        </div>
        <div className="preview-controls">
          <label>
            <span className="sr-only">原创主题</span>
            <select
              aria-label="原创主题"
              value={theme}
          ...
          <label>
            <span className="sr-only">手机内容宽度</span>
            <select
              aria-label="手机内容宽度"
              value={previewWidth}
              onChange={(event) =>
                setPreviewWidth(Number(event.target.value) === 375 ? 375 : 402)
              }
            >
              <option value={375}>375 px</option>
              <option value={402}>402 px</option>
            </select>
          </label>
          <div className="preview-scheme-toggle" aria-label="预览配色">
            <button
              type="button"
        ...
        <div className="preview-content">
          <style
            dangerouslySetInnerHTML={{
              __html: getThemeCSS(theme, previewScheme === "dark"),
            }}
          />
