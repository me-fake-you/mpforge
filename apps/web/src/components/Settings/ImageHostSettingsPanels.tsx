    ...
    <div className="official-host-intro">
      <div className="official-host-summary">
        ...
        <div className="intro-copy">
          <h3>外部自托管上传端点</h3>
          <p>MPForge 不内置上传服务；仅在你明确配置端点后使用</p>
        </div>
        {activeType === "official" && (
          <div className="active-status official-active-status">
            <span className="pulsing-dot"></span>
            <span>当前选择外部端点（未配置时上传会安全失败）</span>
          </div>
        )}
      </div>
      ...
      <div className="host-config">
        <div className="config-field">
          ...
          <input
            type="url"
            placeholder="https://upload.example.test"
            value={viewingConfig.config?.serverUrl || ""}
            onChange={(event) =>
              onConfigChange("serverUrl", event.target.value)
            }
          />
          <small>
            端点需实现 POST /upload 并返回 JSON url。仅允许 HTTPS；localhost
            可使用 HTTP。
          </small>
        </div>
      ...
      <div className="official-feature-list" aria-label="外部端点安全边界">
        <div className="feature-item">
          <div className="feature-icon">
            <Zap size={20} />
          </div>
          <div className="feature-text">
            <strong>用户自主管理</strong>
            <span>服务、域名与数据责任均由端点运营者管理</span>
          </div>
        </div>
        <div className="feature-item">
          <div className="feature-icon">
        ...
        <div className="feature-item">
          ...
          <div className="feature-text">
            <strong>明确配置</strong>
            <span>MPForge 不提供、不背书任何“官方图床”</span>
          </div>
        </div>
      </div>
