...
export function HistoryManager() {
  ...
  const contentArticleActive = Boolean(
    currentFile &&
      /(?:^|[\\/])content[\\/][^\\/]+[\\/]article\.md$/i.test(currentFile.path),
  );
  ...
  useEffect(() => {
    ...
    // Content-as-Code files are persisted by useFileSystem with schema,
    // conflict, approval-invalidation, and audit guarantees. The legacy
    // browser snapshot manager must never replace or snapshot that buffer.
    if (contentArticleActive) return;
  ...
  }, [contentArticleActive, persistActiveSnapshot, saveSnapshot]);
